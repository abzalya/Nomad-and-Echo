...
# file in existence was built with exactly these values.
# Source: python-chess / official Polyglot spec
_RANDOM = [
...
# Piece-index mapping: GameState attr -> Polyglot piece type (0..11)
# Polyglot order: black-P=0, white-P=1, black-N=2, white-N=3,
#                 black-B=4, white-B=5, black-R=6, white-R=7,
#                 black-Q=8, white-Q=9, black-K=10, white-K=11
_PIECE_MAP = (
    ("blackPawns",   0),
    ("whitePawns",   1),
    ("blackKnights", 2),
...
def polyglot_key(gs):
    key = 0
    for attr, piece_idx in _PIECE_MAP:
        bb = getattr(gs, attr)
        for sq in iterateBits(bb):
            key ^= _RANDOM[64 * piece_idx + sq]
    if gs.wKingSideCastle:  key ^= _RANDOM[768]
    if gs.wQueenSideCastle: key ^= _RANDOM[769]
    if gs.bKingSideCastle:  key ^= _RANDOM[770]
    if gs.bQueenSideCastle: key ^= _RANDOM[771]
    if gs.epSquare != -1:
        key ^= _RANDOM[772 + gs.epSquare % 8]
    if gs.whiteToMove:
        key ^= _RANDOM[780]
    return key

# Promotion piece codes in Polyglot: 0=none, 1=knight, 2=bishop, 3=rook, 4=queen
_PROMO_LETTER = {1: "n", 2: "b", 3: "r", 4: "q"}
_PROMO_PIECE_WHITE = {"n": "whiteKnights", "b": "whiteBishops", "r": "whiteRooks", "q": "whiteQueens"}
_PROMO_PIECE_BLACK = {"n": "blackKnights", "b": "blackBishops", "r": "blackRooks", "q": "blackQueens"}

def _decode_move(poly_move, legal_moves, white_to_move):
    to_file   = (poly_move >> 0) & 7
    to_rank   = (poly_move >> 3) & 7
    from_file = (poly_move >> 6) & 7
    from_rank = (poly_move >> 9) & 7
    promo_code = (poly_move >> 12) & 7

    from_sq = from_rank * 8 + from_file
    to_sq   = to_rank   * 8 + to_file

    promo_letter = _PROMO_LETTER.get(promo_code)
    promo_map = _PROMO_PIECE_WHITE if white_to_move else _PROMO_PIECE_BLACK

    for m in legal_moves:
        if m.from_sq != from_sq or m.to_sq != to_sq:
            continue
        if promo_letter:
            if m.promo_piece == promo_map[promo_letter]:
                return m
        else:
...
class PolyglotBook:
    def __init__(self, path):
        self._path = path
        self._n = os.path.getsize(path) // 16

    def probe(self, gs, legal_moves):
        key = polyglot_key(gs)
        with open(self._path, "rb") as f:
            ...
            while lo <= hi:
                mid = (lo + hi) // 2
                f.seek(mid * 16)
                entry_key = struct.unpack(">Q", f.read(8))[0]
                if entry_key == key:
                    mid_match = mid
                    break
                elif entry_key < key:
            ...
            # walk back to the first entry with this key
            start = mid_match
            while start > 0:
                f.seek((start - 1) * 16)
                if struct.unpack(">Q", f.read(8))[0] != key:
                    break
                start -= 1
            ...
            while True:
                ...
                if len(data) < 16:
                    break
                ekey, emove, weight, _ = struct.unpack(">QHHI", data)
                if ekey != key:
                    break
                if weight > 0:
                    candidates.append((weight, emove))

            if not candidates:
                return None

            # pick the highest-weighted move
            _, best_poly = max(candidates)
            return _decode_move(best_poly, legal_moves, gs.whiteToMove)
